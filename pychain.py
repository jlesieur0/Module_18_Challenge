...
# 5. As part of the Add Block button functionality, update `new_block` so that `Block` consists of an 
# attribute named `record`, which is set equal to a `Record` that contains the `sender`, `receiver`, and `amount` values. 
...
amount = st.text_input("Amount Exchanged")
